...
- `services/runner/`  
  **orchestrateur “monde réel”** + **journalisation**.  
  - `journal.py` : écrit `artefacts/episodes.jsonl` (capteurs compactés en base64)  
  - `replay.py` : decode `capteurs_compact` (utile pour l’entraînement offline)
...
- journal réel : `services/runner/app/journal.py`
