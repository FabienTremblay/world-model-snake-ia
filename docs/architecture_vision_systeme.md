...
## agents : catalogue plug-ins (ajout v1)

À partir de la norme « plug-ins », les types d’agents ne sont plus sélectionnés par du code en dur.
Ils sont **déclarés** via des fichiers YAML, puis **chargés dynamiquement** par `agent_service`.

- répertoire scanné : `services/agent_service/app/agents/`
- fichiers découverts : `agent*.yml`
- chaque entrée définit :
  - un `id` (nom de type d’agent)
  - une `fabrique` (callable importable) utilisée pour instancier l’agent

Les prérequis nécessaires à certains agents (ex. `SNAKE_MODELE_JOURNAL`) sont satisfaits via :
- `params` (passés à `creer_agent(id, params=...)`)
